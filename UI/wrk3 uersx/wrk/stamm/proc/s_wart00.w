...
do on error undo, throw:
  ...
  /* set Area MRP Target in newly created MRP Settings of MRP Area 0 */

  if S_Artikel.AenderungZeit = '':U then
  do:

    find bMD_Artikel
      where bMD_Artikel.Firma       = {firma/mlartort.fir S_Artikel.Firma} 
        and bMD_Artikel.Artikel     = S_Artikel.Artikel
        and bMD_Artikel.Lagergruppe = 0
      exclusive-lock no-wait no-error.

    if     available bMD_Artikel
       and bMD_Artikel.AenderungZeit = '':U then

      bMD_Artikel.Bereich = {fnarg
                              pa_cStCchPartTypeArea
                              "S_Artikel.ArtikelArt"}.   

  end. /* if S_Artikel.AenderungZeit = '':U then */
