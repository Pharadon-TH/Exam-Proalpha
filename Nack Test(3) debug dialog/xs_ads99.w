...
DO:
  ...
  /* check condition */
  if input frame {&frame-name} Prefix = '' then
  do:
    adm.method.cls.DMCMessageSvc:prpoInstance:showError
        ('xsads00001':U).
  end. /* if Prefix = '' */ 
  
  if input frame {&frame-name} Number1 = '' or input frame {&frame-name} Number2 = '' then
  do:
    adm.method.cls.DMCMessageSvc:prpoInstance:showError
        ('xsads00002':U).
  end. /* else if Number1 = '' or Number2 = '' */ 
  
  if input frame {&frame-name} Operation <> '+' 
    and input frame {&frame-name} Operation <> '-' 
    and input frame {&frame-name} Operation <> '*' 
    and input frame {&frame-name} Operation <> '/' 
  then
  do:
  ...
  if input frame {&frame-name} tDate = ? then
  do:
    adm.method.cls.DMCMessageSvc:prpoInstance:showError
        ('xsads00004':U).
  end. /* else if tDate = ? */
  
  if input frame {&frame-name} ArtikelFrom > input frame {&frame-name} ArtikelTo then
  do:
    adm.method.cls.DMCMessageSvc:prpoInstance:showError
        ('xsads00005':U).
  end. /* else if ArtikelFrom > ArtikelTo */
  
  
  /* check ArtikelFrom format S_Artikel? */ 
  if not can-find (S_Artikel
  where S_Artikel.Firma   = {firma/sartikel.fir pACConnectionSvc:prpcCompany}
    and S_Artikel.Artikel = input frame {&frame-name} ArtikelFrom )    
    then
    adm.method.cls.DMCMessageSvc:prpoInstance:showError
        ('xsads00006':U).
  ...
  if not can-find (S_Artikel
  where S_Artikel.Firma   = {firma/sartikel.fir pACConnectionSvc:prpcCompany}
    and S_Artikel.Artikel = input frame {&frame-name} ArtikelTo)
